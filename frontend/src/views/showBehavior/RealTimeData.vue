...
// 行为实时页复用传感器页结构，只是换成行为数据源和字段映射。
const tablePrefix = 't_behavior'
...
onMounted(async () => {
  ...
  // WebSocket 重连后主动补拉一次图表，弥补断线期间可能漏掉的数据点。
...
<template>
  <page-container title="实时数据">
    <template #extra>
      <div class="toolbar">
        <el-select v-if="switchStore.value" v-model="selectedDeviceNo" placeholder="请选择编号" clearable
          style="width: 150px">
          <el-option v-for="item in deviceNumbers" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
    ...
    <el-descriptions :column="3" border>
      <el-descriptions-item v-if="switchStore.value" label="编号">
        <el-tag>{{ sensorData.values['编号'] || selectedDeviceNo || '暂无' }}</el-tag>
      </el-descriptions-item>

      <el-descriptions-item v-for="item in sensorData.metadata" :key="item.f_name"
      ...
      <el-descriptions-item label="视频或图片">
        <div v-if="mediaInfo" class="media-cell">
          <el-image v-if="!mediaIsVideo" :src="mediaInfo.thumb_url || mediaInfo.media_url"
            :preview-src-list="[mediaInfo.media_url]" fit="cover" class="media-thumb" />
          <video v-else class="media-thumb" :src="mediaInfo.media_url" muted playsinline
    ...
    <el-dialog v-model="previewVisible" title="视频预览" width="70%">
      <video v-if="mediaIsVideo && mediaInfo" :src="mediaInfo.media_url" controls autoplay style="width: 100%" />
    </el-dialog>
  </page-container>
</template>
