...
### 2026-09-08 温度传感器装反判定与可配置确认时间保护已实现

- **物理判据与异常识别**：
  - 水流经加热器升温，正常加热工况下入口水温必须小于等于出口水温（$T_{in} \le T_{out}$）；
  - 当加热器开启（`heat_Y1 = on` 或期望开启）且水泵正常运转具备安全流量（$flow \ge min\_safe\_flow$）时，若检测到入口温度大于出口温度（$T_{in} > T_{out} + 0.1$），启动装反计时；
  - 持续超过配置的确认时间（`temp_reversed_confirm_time`，默认 5 秒）后确诊为进出口温度传感器装反故障（`TEMP_SENSOR_REVERSED`）。
- **保护动作与状态联动**：
  - 切断加热器（MQTT发布关加热）；若管路流量和压力处于安全区间，则保持水泵继续运转循环散热（留泵散热），状态机锁定为 `FAULT`；
  - 报警记录持久化至 `t_error_msg`（错误码 `TEMP_SENSOR_REVERSED`，错误信息“进出口温度传感器疑似装反(入口XX℃ > 出口XX℃，持续超过Xs)”），同时 WebSocket 实时向前端广播错误流；
  - 人工复位故障（`resetFault`）时重置装反计时，复位前若装反工况未解除则拒绝复位。
- **参数配置与页面联动**：
  - `t_direct_config` 种子与控制引擎新增 `temp_reversed_confirm_time`（默认 5 秒）；
  - 前端 `controlValidation.js` 白名单已支持该参数数字输入与合法性校验，在【指令信息】页自动控制树下可自由配置；
  - 热工效能分析服务（`thermalAnalysisService.js`）同步联动：两水箱温差幅度维持绝对值展示，供热温差保留 $T_{out} - T_{in}$ 方向；装反工况下热功率估算置 0，功率状态标注为 `reversed`（“温度传感器疑似装反”）。
- 后端 144 项测试、前端 25 项测试全部通过，生产构建通过。
