...
### 2026-09-09 失流干烧与水泵空转彻底解耦及联动参考最低压力已实现

- **物理机理与故障分类解耦**：
  - 彻底厘清“建流超时”、“水泵空转”与“失流干烧”三者的物理语义差异：
    1. **启动建流超时（`BUILD_FLOW_TIMEOUT`）**：开泵初期的流量爬升缓冲期（`build_flow_timeout`，默认 5 秒）。若超时流速仍未达到最低安全流量（`min_safe_flow`，默认 0.5 L/min），系统坚决禁止开启加热，关泵停机报警，保护加热管不被干烧；
    2. **水泵空转保护（`PUMP_IDLING`）**：水泵运转中，低压低流（压力 $P < min\_operating\_pressure$ 且流量 $Q < min\_safe\_flow$），明确为水泵抽空、吸不上水空磨，即刻强制停泵保护电机；
    3. **失流干烧保护（`LOW_FLOW`）**：**仅在加热器开启**（`heaterState === 'on'` 或 `desiredHeaterState === 'on'`）工况下，若循环水断流或过低（$Q < min\_safe\_flow$），才确认为干烧故障；加热器未开启时坚决不报“干烧”；
- **配置复用与零配置负担**：
  - 空转低压判定完全复用指令页面已有的【参考最低压力】（`min_operating_pressure`，默认 20.0 kPa），不增加冗余配置项；
  - 自动运行模式与手动单开水泵工况均接入低压低流研判，常压低流（$P \ge 20\text{kPa}$）不误判为空转。
- **错误语义与字典对齐**：
  - `errorMessageMapping.js` 增加 `PUMP_IDLING` 映射（“水泵空转保护”）；
  - 更新 `BUILD_FLOW_TIMEOUT` 语义为“启动建流超时(流速未达标禁止加热)”，消除与空转的文案混淆；
  - 自动模式建流超时描述更新为“启动未建流(超过Xs未达到最低流量，流速未达标禁止加热)”。
- 后端 153 项测试、前端 25 项测试全部通过，生产构建通过。


- **物理判据与双维干烧闭环**：
  - 区别于基于循环流量的“失流干烧”（`LOW_FLOW`），实现基于热工放热特性的“加热无温升干烧”（`DRY_HEATING_NO_TEMP_RISE`）；
  - 当加热器开启（`heaterState === 'on'` 或 `desiredHeaterState === 'on'`）且水流正常循环中，动态累计通电放热时长；
  - 若在配置的判定时间（`dry_heating_timeout`，默认 15 秒）内，出口温度相对于基准温度的温升 $\Delta T < dry\_heating\_temp\_diff$（默认 0.2℃），确诊发热管空烧、断线虚接或温度探头脱落故障；
  - 若出口水温正常上升（$\Delta T \ge dry\_heating\_temp\_diff$），自动推进基准水温并清零累计时间，彻底避免正常升温时的误报；
  - 兼容回差控温持续长开与时间比例 PID 脉冲加热工况，以实际通电放热时间为统计标尺。
- **保护动作与留泵散热**：
  - 确诊干烧瞬间立即发布关闭加热指令（`heater = off`），撤销加热调度许可；
  - 若管路流量和压力处于安全区间，则保持水泵继续运转循环流水散热（留泵散热），状态机锁定为 `FAULT` 态；
  - 审计留痕：向 `t_error_msg` 写入故障记录（错误码 `DRY_HEATING_NO_TEMP_RISE`，描述“加热无温升干烧保护(加热累计Xs温升仅X.XX℃ < X.X℃...)”），同时通过 WebSocket 广播 `error_realtime` 与 `alarm_realtime`。
- **指令页面配置联动**：
  - `t_direct_config` 种子中扩展增加 `dry_heating_timeout`（无温升干烧判定时间（秒），默认 15 秒）与 `dry_heating_temp_diff`（无温升干烧判定温差（℃），默认 0.2℃）；
  - 前端 `controlValidation.js` 白名单已支持两个参数的数值校验，在【指令信息】页自动控制树下自动渲染并支持自由编辑与单设备覆盖。
- 后端 152 项测试、前端 25 项测试全部通过，生产构建通过。

### 2026-09-08 基于建流时间豁免的水泵空转研判与即时强制停泵保护已实现

- **研判时机与爬升豁免**：
  - 水泵开启（无论手动模式还是自动模式）后，系统以前 `build_flow_timeout`（建流超时时间，默认 5 秒）作为流量爬升缓冲期，**豁免不判定为空转**；
  - 超过建流时间后，若实时水流仍然低于 `min_safe_flow`（最低安全流量，默认 0.5 L/min），即刻确认为**水泵空转**。直接复用现有参数，不增加冗余配置。
- **保护动作与模式响应**：
  - **即刻强制停泵**：不设二次倒计时，确诊空转瞬间应用层立即向 MQTT 下发关闭水泵指令（`pump = off`），坚决阻断电机干磨损坏；
  - **自动模式（RUNNING / BUILDING_FLOW）**：建流超时触发 `BUILD_FLOW_TIMEOUT` 故障，切断加热、停泵并锁定为 `FAULT` 态；
  - **手动模式（STOPPED）**：仅执行保护性关泵，开关自动回滚归位，**不锁死界面**；操作员检查管路和水源后可直接再次手动点击开泵，新一轮开泵将重新获得建流豁免周期。
- **告警通知与审计留痕**：
  - 通过 WebSocket `alarm_realtime` 与 `error_realtime` 向前端广播报警，前端弹出全局 `ElNotification` 错误通知；
  - 审计留痕：在 `t_direct_history` 记录应用层指令下发（关泵，备注“水泵空转保护自动关泵”）；
  - 错误日志：向 `t_error_msg` 写入故障报警（关联水力联合诊断过程二 `HYDRAULIC_PUMP_ABNORMAL`）。
- 后端 149 项测试、前端 25 项测试全部通过，生产构建通过。
...
### 2026-09-09 顶部故障状态布局稳定
- 顶部设备状态列表固定单行并限制可用宽度，长故障文案使用省略号，悬停可查看全文；设备编号、状态点及右侧告警声音开关不再被长文案挤动或换行。
- 前端25项测试通过，并使用1558×1015无头浏览器实页截图检查；未运行生产构建。

### 2026-09-09 比赛方智能判定模型调查
- v3 大纲明确组委会现场提供 1 套“基于 HTTP 接口的智能判定服务软件”，v3 细则明确现场 U 盘用于分发竞赛所需模型资源。
- 现有官方材料未指定 YOLO、Python、权重格式、数据集、请求字段或响应字段；接入时应视为本地 HTTP 黑盒服务，以现场纸质任务书和 U 盘说明为准。
- 调查报告见 `比赛方AI模型接入调查.md`。当前项目已有可配置 URL、请求模板和 10 秒超时的转发骨架；响应映射、判定结果展示及行为/错误数据落库仍需等待现场协议后完成。
- 上一届资料 `yolo_model/yolo11-flask.py` + `yolo11n.pt` 是 Flask 图像检测服务：`POST http://127.0.0.1:5000/infer`，输入字段为 Base64 `image`，返回检测列表和画框图片。当前传感器行 `{rows}` 接口与它不兼容；默认 Python 3.13 当前缺少 `ultralytics`。
