...
- 传感器入库动态校验与历史状态检索：
  - 新建 `sensorValidationService` 动态安全阈值判定模块：入库时依据当前设备设定的 `max_safe_temperature`、`max_safe_pressure` 等参数动态计算 `vstatus`；超温或超压时标记为 `1`（告警），正常时标记为 `0`；具备短时内存缓存兼顾高频入库吞吐与配置即刻生效；
